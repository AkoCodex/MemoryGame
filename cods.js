...
const mainMenuButton = document.querySelector(".game__menu__mainmenu__button");
...
const secondPlayer = document.querySelector(".game__menu__points__player2__title")
const player1ResultDisplay = document.querySelector(".game__menu__points__player1__result");
const player2ResultDisplay = document.querySelector(".game__menu__points__player2__result");
const remainingMovesDisplay  = document.querySelector(".game__menu__moves__result");
...
function startGame(count) {
    ...
    cardArray = ([...allCardArray].slice(0,count)).sort(() => 0.5 - Math.random());
...
function createBoard() {
    ...
    for(let i = 0; i < cardArray.length; i++) {
        ...
        card.setAttribute("src", "imagens/virada.png");
...
// Verifica quantidade de cartas selecionadas
  if(isTriple == false) {
    ...
    isBoardLocked = true;
    setTimeout(() => {checkForMatch(); if(remainingMoves > 0) { isBoardLocked = false }}, 500);
   }
  }
  if(isTriple == true) {
...
function checkForMatch() {
    ...
    if(isTwoPlayers == false) {
        ...
        if(cardsChosen[0] === cardsChosen[1]) {
            ...
            cards[optionOneId].setAttribute("src", "imagens/virada.png");
            cards[optionTwoId].setAttribute("src", "imagens/virada.png");
        }
        cardsChosen = [];
        cardsChosenId = [];
    ...
    if(isTriple == true) {
        ...
        if(cardsChosen[0] === cardsChosen[1] && cardsChosen[0] === cardsChosen[2]) {
            cards[optionOneId].style.visibility = "hidden";
            cards[optionTwoId].style.visibility = "hidden";
            cards[optionTreeId].style.visibility = "hidden";
            cardsWon.push(cardsChosen);
        }
        else {
            cards[optionOneId].setAttribute("src", "imagens/virada.png");
            cards[optionTwoId].setAttribute("src", "imagens/virada.png");
            cards[optionTreeId].setAttribute("src", "imagens/virada.png");
        }
        cardsChosen = [];
        cardsChosenId = [];
        resultDisplay.textContent = cardsWon.length;
    ...
    if(isTwoPlayers) {
        ...
        if(player1CardsWon.length === cardArray.length/2) {
            player1ResultDisplay.textContent = "Parabéns, você encontrou todos os pares!!"
            player2ResultDisplay.textContent = "Que pena, mais sorte da próxima vez!"
            restart.style.display = "initial";
        }
        if(player2CardsWon.length === cardArray.length/2) {
            player1ResultDisplay.textContent = "Que pena, mais sorte da próxima vez!"
            player2ResultDisplay.textContent = "Parabéns, você encontrou todos os pares!!"
            restart.style.display = "initial";
        }
        
        if(player1CardsWon.length + player2CardsWon.length == 12) {
            if(player1CardsWon.length > player2CardsWon.length) {
                player1ResultDisplay.textContent = "Parabéns, você encontrou a maioria do pares!!"
                ...
                player2ResultDisplay.textContent = "Parabéns, você encontrou a maioria do pares!!"
                restart.style.display = "initial";
            }
        }
    }
...
function deleteCards() {
    let cardsToDelete = document.querySelectorAll("img");
...
twelveCards.addEventListener("click", () => { result.style.display = "initial";
...
eighteenCards.addEventListener("click", () => { result.style.display = "initial";
    moves.style.display = "initial";
    player1Result.style.display = "none";
    player2Result.style.display = "none";
    remainingMoves = 22;
...
twentyFourCards.addEventListener("click", () => { result.style.display = "initial";
    moves.style.display = "initial";
    player1Result.style.display = "none";
    player2Result.style.display = "none";
    remainingMoves = 30;
...
triple.addEventListener("click", () => { result.style.display = "initial";
    moves.style.display = "initial";
    player1Result.style.display = "none";
    player2Result.style.display = "none";
    isTriple = true;
...
twoPlayers.addEventListener("click", () => { result.style.display = "none";
...
twoPlayersTriple.addEventListener("click", () => { result.style.display = "none";
    moves.style.display = "none";
    player1Result.style.display = "initial";
    player2Result.style.display = "initial";
    player1Result.style.color = "white";
